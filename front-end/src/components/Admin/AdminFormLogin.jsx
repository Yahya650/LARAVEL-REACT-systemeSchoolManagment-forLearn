...
const AdminFormLogin = () => {
    ...
    const submitForm = async ({ email, password }) => {
        await auth.login({ email, password, guard: 'admin' })
    };
                    ...
                    {isLoading && <p className="alert alert-primary py-1 my-3 mt-5" role="alert">Form is Loading...</p>}
                    {
                        !isLoading && <form onSubmit={handleSubmit(submitForm)} className='p-5' style={{ border: '1px solid black', borderRadius: '10px', backgroundColor: '#f8f9fa' }}>
                            ...
                            <div className="mb-3">
                                ...
                                <input {...register('email')} disabled={isSubmitting} type="text" className={`form-control ${errors.email || auth.errors.email ? "is-invalid" : ""}`} id="Emailaddress" aria-describedby="emailHelp" />
                            ...
                            <button type="submit" disabled={isSubmitting} className="btn btn-primary me-1">{isSubmitting && 'Loading...'} {!isSubmitting && 'Submit'}</button><Link className="link-opacity-25-hover" to={"/admin/register"}>if you dont have account click here for create it</Link>
                            <br />
                            <Link className="link-opacity-25-hover" to={'/admin/forgot-password'}> Forgot Password ? </Link>
                        </form>
                    }
                </>
            }
